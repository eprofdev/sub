...
**الوضع التلقائي هو المقصود:** حين يكون النفق العام لا يصل إلى Cloudflare إلا
عبر الـ VPN، فالمطلوب أن يسلك المرور الـ VPN لا أن يتجاوزه. لذلك:

| حالة مسار الـ VPN | ما يفعله |
|---|---|
| يعمل | **لا قواعد** — مرور الراوتر يسلك الـ VPN كما ضبطته |
| ساقط | يضع العلامة `0x8000` فيخرج مباشرة، وإلا تعطّل كل شيء |
| عاد | يرفع القواعد فورًا ويعيد تشغيل cloudflared ليتصل عبر الـ VPN |

الفحص لا يعطّل شيئًا: يقرأ المسار الافتراضي في جداول السياسة، ثم يختبر حياة
النفق — ping لبوابته، أو آخر مصافحة لـ WireGuard إن كان `wg`. القرار يُتخذ من
