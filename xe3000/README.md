...
| بوابة مكتملة | ترقية في المكان بالإعدادات المثبّتة كما هي |
| بيانات Cloudflare محفوظة | تثبيت كامل بلا أي إدخال |
| لا شيء محفوظ + طرفية | تثبيت تفاعلي يسأل عن القيم الأربع |
| لا شيء محفوظ بلا طرفية | فتح صفحة الإعداد على `http://192.168.8.1:9000/cgi-bin/setup.cgi` |

### الترقية فوق تثبيت قائم

```sh
sh /root/xe3000autouiinput.sh upgrade
```

يقرأ `state/settings.env` الموجود ويبني منه كل شيء: إعداد xray وcloudflared،
وملفي الخدمة، وملف المراقبة وتجاوز كِل‑سويتش VPN إن كانا مفعّلين، ثم اللوحة
والاختصارات. **لا يُكتب فوق الإعدادات ولا المستخدمين ولا بيانات الاعتماد، ولا
يُلمس Cloudflare إطلاقًا** — لا نفق جديد ولا سجل DNS جديد.

لذلك تبقى كما هي: اسم المضيف، معرّف النفق، مسار WebSocket، المنفذ المحلي،
الناقل والبروتوكول، ومعرّفات كل المستخدمين — **روابط العملاء تظل صالحة بلا
إعادة استيراد**. في نهاية الترقية يقارن السكربت `settings.env` و`users.tsv`
ببصمتهما قبلها ويخبرك إن تغيّر شيء.

الأمر يرفض العمل إن لم يكن هناك تثبيت مكتمل، أو إن كانت الإعدادات المحفوظة بلا
اسم مضيف أو بلا معرّف نفق، أو إن كان ملف اعتماد النفق مفقودًا — فلا يبني فوق
إعداد ناقص.

وإن كان `cloudflared` و`xray` موجودَين فلا يُنزَّل شيء من الإنترنت، فالترقية
تعمل على راوتر بلا خروج.
...
## ذاتي الإصلاح والضبط والتحديث

```sh
xe3000 doctor        # يكتشف الأعطال المعروفة ويُصلحها بلا تدخّل
xe3000 autotune      # يجرّب التركيبات ويُبقي أول ما ينجح
xe3000 selfupdate    # يحدّث نفسه بعد تحقق البصمة ثم يشغّل doctor
xe3000 watchdog on 5 # مراقبة كل 5 دقائق: doctor ثم إعادة تشغيل عند السقوط
```

**`doctor`** يعالج كل عطل واجهناه فعلًا: ملف خدمة بلا `GODEBUG=multipathtcp=0`،
وTCP Fast Open مفعّلًا، ونسخة uhttpd قديمة تحجز المنفذ، ولوحة بلا مولّد QR بعد
ترقية، وقاعدة جدار ناري مفقودة، وكِل‑سويتش VPN بلا تجاوز، وخدمة متوقفة. وما يحتاج
قرارك يُعرض ولا يُغيَّر من تلقائه.

**`autotune`** يجرّب `127.0.0.1/ws` ثم `LAN/ws` ثم `unix/xhttp` ثم `127.0.0.1/xhttp`،
ويقيس كل تركيبة محليًا وعبر Cloudflare، ويحفظ أول ما ينجح — ويعيد الإعداد السابق إن
لم تنجح أي واحدة.

**`selfupdate`** ينزّل النسخة وبصماتها، **يرفض التثبيت إن لم تطابق البصمة أو إن كان
في النسخة خطأ صياغة**، يحتفظ بنسخة `.bak`، ثم يشغّل `doctor`. البصمة تأتي من نفس
المصدر، فهي تحمي من التلف والانقطاع لا من اختراق المصدر نفسه.
...
> **المنفذان يعملان معًا:** `http://192.168.8.1:9000/` و`https://192.168.8.1:9443/` كلاهما
> يفتح اللوحة، بلا تحويل قسري. لتغيير منفذ HTTPS:
> `FULLTUNNEL_UI_PORT_HTTPS=9444 sh xe3000autouiinput.sh ui-only`

> **الحماية معطّلة افتراضيًا:** اللوحة تُفتح مباشرة من شبكتك المحلية. في أعلى الصفحة
> بطاقة «حماية اللوحة» فيها حقلا اسم المستخدم وكلمة المرور وزر تفعيل، وزر تعطيل بعدها.
> ومن السطر: `xe3000 auth on` · `xe3000 auth off` · `xe3000 auth status`.
> بلا حماية، **أي جهاز على شبكتك يرى روابط الاشتراك ويديرها** — الجدار الناري يمنع
> الوصول من خارج LAN، لا من داخلها.
...
يُثبَّت الأمران `menu` و`xe3000` في `/usr/bin` ضمن الخطوة `[6/6]`. إن كان
`/usr/bin/menu` محجوزًا لبرنامج آخر فلن يُستبدل، ويبقى `xe3000 menu`.

ترويسة القائمة تعرض المضيف وعدد المستخدمين والناقل والبروتوكول وعنوان الاستماع
وحالة الخدمتين والمراقبة الدورية وحماية اللوحة. ثم:

| الخيار | ماذا يفتح |
|---|---|
| 1 | الحالة |
| 2 | المستخدمون: إضافة، حذف، تطبيق |
| 3 | الروابط |
| 4 | تشغيل/إيقاف/إعادة تشغيل الخدمتين |
| 5 | فحص السلسلة الكامل (selftest) |
| 6 | الصيانة الذاتية: `doctor`، `autotune`، `selfupdate`، وتفعيل/تعطيل/تجربة المراقبة |
| 7 | الإعدادات: الناقل، البروتوكول، الاستماع، المنفذ، جسر SSH، تجاوز كِل‑سويتش VPN، تبديل التوكن |
| 8 | حماية اللوحة: تفعيل باسم وكلمة مرور، أو تعطيل، أو عرض العنوان |
| 9 | تثبيت/إكمال |

كل إجراء يُنفَّذ في صدفة فرعية، فخطأ في أمر واحد يطبع سببه ويعيدك إلى القائمة بدل
أن يُنهيها.
...
sh xe3000autouiinput.sh upgrade          # ترقية تثبيت قائم بإعداداته كما هي
...
## الأسرار لا تغادر الراوتر

| البيان | مكانه | يصل إلى GitHub؟ |
|---|---|---|
| توكن Cloudflare و Account/Zone ID | `/etc/xe3000-cf-fulltunnel-creds/` (700/600) | **لا** |
| سرّ النفق | `state/../tunnel/<id>.json` (600) | **لا** |
| معرّفات المستخدمين وكلمات مرورهم | `state/users.tsv` (600) | **لا** |
| المضيف ومسار WS | `state/settings.env` (600) | **لا** |
| كلمة مرور اللوحة | `ui/httpd.conf` (600، مُعمّاة) | **لا** |

المستودع يحوي السكربتات والتوثيق فقط. المُثبّت لا يتصل إلا بـ `api.cloudflare.com`
وبمضيفك وبالعنوان المحلي، ويُنزّل cloudflared من إصدار Cloudflare الرسمي — ولا يرسل
شيئًا إلى أي جهة أخرى، ولا يوجد فيه أي إرسال تتبّع.

**الناتج التشخيصي:** `selftest` و`status` و`diagnose` **تُخفي** المضيف ومعرّف النفق
والمسار افتراضيًا، لأن ناتجها يُلصق كثيرًا في محادثات ومنتديات. لإظهارها لنفسك:

```sh
FULLTUNNEL_SHOW_SECRETS=1 sh xe3000autouiinput.sh status
```

`links` وحده يطبع الروابط كاملة — فذلك غرضه.

يمنع فحص في CI وصول أي معرّف حقيقي أو قيمة شبيهة بتوكن إلى `xe3000/`.
