...
### أريد العكس: كل شيء **داخل** الـ VPN

```sh
sh /root/xe3000autouiinput.sh vpn-route on
sh /root/xe3000autouiinput.sh vpn-route status
```

`vpn-bypass off` وحده **لا يكفي** على GL.iNet: سياستها تمرّر أجهزة الشبكة عبر
الـ VPN وتترك ما ينشئه الراوتر يخرج مباشرة إلى المشغّل. القاعدة المقصودة موجودة
لديها بأولوية `90019` — أي **بعد** `32766: lookup main` فلا تُستعمل أبدًا.

`vpn-route on` يركّب ثلاث قطع، ولا واحدة تكفي وحدها:

| القطعة | لماذا |
|---|---|
| `ip rule pref 7000 iif lo lookup <جدول النفق>` | أولوية بين 6000 (حيث تخرج حزم الـ VPN المشفّرة نفسها فلا يحدث دوران) و32766 |
| `ip -6 rule pref 7000 …` | **قواعد IPv6 منفصلة تمامًا.** بدونها يتسرّب مرور IPv6 إلى المشغّل، ويظهر كأعطال متفرقة: `wget` يحاول عنوان IPv6، وcloudflared يحصل على `network is unreachable` من عناوين حافة IPv6 |
| مُحلِّل DNS يصل داخل النفق | مُحلِّلات المشغّل غالبًا عناوين IPv6 لا وجود لها داخل النفق، فيصير الخطأ `Could not resolve host` بعد أن ينضبط التوجيه |

يكتشف جدول النفق وحده (أي جدول مساره الافتراضي عبر `wg*` أو `tun*`)، ويتحقق بعد
التطبيق أن `ip route get` يخرج فعلًا عبر الواجهة المتوقعة، ويبدّل المُحلِّل **فقط**
إن كان الحالي لا يترجم. ويُثبَّت في `include` الجدار الناري فيصمد بعد الإقلاع —
قواعد `ip rule` تزول بإعادة التشغيل.

`vpn-route off` يزيل القاعدتين ويعيد `resolv.conf` كما كان.

بعده أعد تشغيل النفق ليطلب المسار الجديد:
`/etc/init.d/xe3000-cf-tunnel restart`

#### الفرق عن `vpn-bypass`

نقيضان: `vpn-bypass` يُخرج مرور البوابة **من** الـ VPN (حين يكون الـ VPN هو
العائق)، و`vpn-route` يُدخله **فيه** (حين يكون المشغّل هو العائق). لا تفعّلهما
معًا.

### ملاحظة: `vpn-bypass off` وحده

حين يكون المشغّل هو مصدر القيد — حجب منافذ، أو باقة تسمح ببعض التطبيقات دون
غيرها — فالمطلوب ألا يرى المشغّل إلا حزم VPN مشفّرة. وهذا هو الوضع `off`:

```sh
sh /root/xe3000autouiinput.sh vpn-bypass off
```

سياسة GL.iNet تدفع كل ما ينشئه الراوتر إلى الـ VPN أصلًا، فـ `off` يعني ببساطة
**لا تتجاوزها**. وهو الخيار الصحيح للخصوصية أيضًا: مع `on` يخرج DNS والمنافذ
443 و7844 **خارج** الـ VPN فيراها المشغّل.

للتأكد أن المرور يسلكه فعلًا لا ظنًّا:

```sh
sh /root/xe3000autouiinput.sh vpn-bypass status
```

سطر **الخروج** يقرأ قرار النواة نفسه ويقول من أي واجهة يخرج مرور النفق:
`wg0` يعني WireGuard، و`tun0` يعني OpenVPN، و`rmnet_mhi0` أو `eth0` يعني أنه
يخرج مباشرة إلى المشغّل. وينبّهك إن كان التجاوز معطّلًا والخروج ليس عبر نفق.

> **لا يعمل على خط بلا باقة.** إن وضعك المشغّل في حديقة مسوّرة (القسم السابق)
> فلن يخترقها WireGuard أيضًا — فهو UDP على منفذ غير مألوف. أعد الخدمة للخط أولًا.

> **وتأكد أن خادم الـ VPN يمرّر المنفذ 7844.** رأينا خادم OpenVPN يقطع مصافحة
> TLS مع حافة Cloudflare عليه (`connection reset` متكرر) بينما 443 يعمل. إن فعل
> خادمك ذلك فلن يقوم النفق عبره، مهما كان كل شيء آخر سليمًا.

| الوضع | متى |
|---|---|
| `auto` | الافتراضي — الـ VPN مفضّل، والتجاوز عند فشل النفق عبره |
| `on` | تجاوز دائم؛ لن يسلك المرور الـ VPN حتى وهو يعمل |
| `off` | **كل شيء عبر الـ VPN** — الصحيح حين يكون المشغّل هو مصدر القيد |
